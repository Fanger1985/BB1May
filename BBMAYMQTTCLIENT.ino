...
const char* mqtt_broker = "192.168.1.1"; // Change this to the IP of your MQTT broker
...
void callback(char* topic, byte* message, unsigned int length) {
    ...
    // Handle message depending on your topic
}
...
void setup() {
    Serial.begin(9600);
    setupWiFi(); // Your function to connect to WiFi
...
void loop() {
    ...
    // Handle HTTP web server client requests
    server.handleClient();

    // Process sensor readings and autonomous actions
    manageRobotOperations();

    // Publish sensor data at regular intervals
    publishSensorData();
}

void manageRobotOperations() {
    // Read MPU6050 sensor data
    mpu.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);

    // Check for sudden movements to ensure robot safety
    checkForSuddenMovement();

    // Handle autonomous or manual control based on mode
    if (isManualControl) {
        // Manual control actions are likely handled by web server routes
    } else {
        // Autonomous operations such as navigation and obstacle avoidance
...
void publishSensorData() {
    ...
    if (currentMillis - lastPublishTime > 1000) {  // Publish every second
        lastPublishTime = currentMillis;

        if (client.connected()) {
            StaticJsonDocument<200> doc;
            doc["IR_left"] = digitalRead(IR_LEFT);
            doc["IR_right"] = digitalRead(IR_RIGHT);
            ...
            client.publish("robot/sensors", output.c_str());
        }
    }
}

void checkForSuddenMovement() {
    if (abs(ax) > 15000 || abs(ay) > 15000 || abs(az) > 15000) {
        Serial.println("Sudden movement detected! Stopping motors.");
        stopMotors();  // Implement motor stop function to halt movement immediately
    }
}

void performAutonomousActions() {
    int pirState = digitalRead(PIR_SENSOR_PIN);
    if (pirState == HIGH) {  // Human detection logic
        handleHumanDetection();
    } else {
        Serial.println("Exploring the environment...");
        autoMove();  // Engage in autonomous exploration
    }
}

void handleHumanDetection() {
    Serial.println("Human detected! Expressing happiness...");
    expressEmotion("happy");

    int followDistance = getUltrasonicDistance();
    ...
    updateMap();  // Update environmental mapping
}
...
// Function to add data to a circular buffer
void addDataToBuffer(int distance, String action) {
    if (bufferIndex >= MAX_BUFFER_SIZE) bufferIndex = 0; // Reset index if it exceeds buffer size
